...
const NewBlog = ({ title }) => {
  ...
  const changeStatusComplete = async (order) => {
    if (!order) {
      console.log("Order is undefined or null!");
      return;
    }
    
    console.log("Function triggered for order:", order); // Check if function runs
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/updateOrderStatus`, {
        ...
        body: JSON.stringify({ orderId: order._id, isCompleted: true }),
      });
      ...
      setOrders((prevOrders) =>
        prevOrders.map((o) =>
          o._id === order._id ? { ...o, isCompleted: true } : o
        )
      );
    } catch (error) {
      console.error('Error updating order status:', error);
  ...
  className="approve-btn"
  onClick={() => {
    console.log("Button Clicked:", order); // Check if button is clicked
    changeStatusComplete(order);
  }}
  disabled={order.isCompleted=="completed"} // Disable if already completed
>
  {order.isCompleted=="Completed" ? "Completed" : "Mark as Completed"}
</button>
