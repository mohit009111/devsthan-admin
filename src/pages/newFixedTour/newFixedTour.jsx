...
const NewDestination = ({ title }) => {
  ...
  const handleItineraryChange = (index, field, value, category) => {
    ...
    // Ensure that `activities` exists before modifying it
    if (field === "activities") {
      updatedItineraries[index][field] = value.length > 0 ? value : undefined;
    } else {
      updatedItineraries[index][field] = value;
    }
  ...
  const handleDestinationSelect = (destinationId) => {
    ...
    setTourData((prevTourData) => ({
      ...prevTourData,
      destinationId: selectedDestination ? selectedDestination.uuid : "",
    }));
  };
  const handleBannerImageChange = (event) => {
    ...
    if (file) {
      if (tourData.bannerImage) {
        URL.revokeObjectURL(tourData.bannerImage);
      }
  ...
  const handleFileChange = (event) => {
    ...
    console.log("Uploaded Images:", previewUrls);
  };
